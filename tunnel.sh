...
if [ -z "$NODE" ]; then
    if squeue -u $USER -O name:32|grep code; then
        ...
        nohup srun -N 1 -n $CPUS -p $PARTITION --mem="$MEMORY"g --gres=gpu:$GPUS -t $TIME:00:00 ~/code tunnel --accept-server-license-terms --name $PARTITION > ~/logs/tunnel.log 2>&1 &
    fi
else
    if squeue -u $USER -O name:32|grep code; then
        echo "Tunnel is already running."
        ...
        nohup srun -N 1 -n $CPUS -p $PARTITION --mem="$MEMORY"g --gres=gpu:$GPUS -t $TIME:00:00 -w $NODE ~/code tunnel --accept-server-license-terms --name $PARTITION > ~/logs/tunnel.log 2>&1 &
    fi
fi
ENDSSH
